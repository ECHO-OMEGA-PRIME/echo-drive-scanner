...
def extract_schemas(content: str, path: Path, lang: str) -> list[dict]:
    ...
    if lang == 'python':
        ...
        for m in re.finditer(r'class\s+(\w+)\s*\(\s*(BaseModel|TypedDict|dataclass)[^)]*\)', content):
            results.append({
                'file_path': str(path),
...
def extract_api_endpoints(content: str, path: Path, lang: str) -> list[dict]:
    ...
    results = []
    # Express / Hono routes
    for m in re.finditer(r'''(?:app|router)\.(get|post|put|delete|patch)\s*\(\s*['"`]([^'"`]+)['"`]''', content, re.I):
        results.append({
            'file_path': str(path),
...
def extract_prompts(content: str, path: Path) -> list[dict]:
    ...
    for m in re.finditer(r'(?:system_prompt|SYSTEM_PROMPT|system_message|instructions)\s*[=:]\s*[f]?["\'\`]{1,3}(.*?)["\'\`]{1,3}', content, re.DOTALL):
    ...
    # {"role": "system", "content": "..."}
    for m in re.finditer(r'"role"\s*:\s*"system"[^}]*"content"\s*:\s*"([^"]{50,})"', content, re.DOTALL):
        results.append({
            'file_path': str(path),
...
def extract_configs(content: str, path: Path, lang: str) -> list[dict]:
    ...
    if lang == 'python':
        # ALL_CAPS = value
        for m in re.finditer(r'^([A-Z][A-Z0-9_]{3,})\s*(?::\s*\w+\s*)?=\s*(.{1,100})$', content, re.MULTILINE):
            results.append({
                'file_path': str(path),
                'key': m.group(1),
    ...
    for m in re.finditer(r'(?:os\.environ|process\.env)\[?\s*["\']?([A-Z][A-Z0-9_]{3,})["\']?\s*\]?', content):
        results.append({
            'file_path': str(path),
            'key': m.group(1),
